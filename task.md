...
Возьмите датасет из домашки по ClickHouse (таблица `transactions_db.transactions` с данными о транзакциях и признаком фрода) и реализуйте на его основе проект dbt. Я взял transactions.csv  - это результат команды из прошлой дз

```
docker exec clickhouse clickhouse-client \
  --query "SELECT * FROM transactions FORMAT CSVWithNames" > transactions.csv
```
